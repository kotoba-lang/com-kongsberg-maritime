(ns kongsberg_maritime.main
  "Kotodama WASM entrypoint for the Kongsberg_maritime clean-room actor (L4) — Clojure port.
  ...
  facts (`kongsberg_maritime.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kongsberg_maritime")
(def tier "L4")
...
(def entity-specs
  [{:entity "Object"       :plural "objects"       :id-prefix "kongsber_obj"
    :fields [:designator :type :source]                :required [:designator :type]
    :coerce {}                                :refs {}}
   {:entity "Position"     :plural "positions"     :id-prefix "kongsber_pos"
    :fields [:objectId :lat :lon :altitudeM :asOf]     :required [:lat :lon]
    :coerce {:lat :float :lon :float :altitudeM :float} :refs {:objectId "Object"}}
   {:entity "Observation"  :plural "observations"  :id-prefix "kongsber_obs"
    :fields [:objectId :parameter :value :observedAt]  :required [:parameter :value]
    :coerce {:value :float}                   :refs {:objectId "Object"}}
   {:entity "Scene"        :plural "scenes"        :id-prefix "kongsber_sce"
    :fields [:satellite :sceneId :cloudCover :capturedAt] :required [:satellite :cloudCover]
    :coerce {:cloudCover :float}              :refs {:sceneId "Scene"}}
   {:entity "Float"        :plural "floats"        :id-prefix "kongsber_flo"
    :fields [:wmoId :lat :lon :parkDepthM]         :required [:lat :lon]
    :coerce {:lat :float :lon :float :parkDepthM :float} :refs {}}
   {:entity "Station"      :plural "stations"      :id-prefix "kongsber_sta"
    :fields [:stationId :name :network]            :required [:name :network]
    :coerce {}                                :refs {:stationId "Station"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kongsberg_maritime.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kongsberg_maritime-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kongsberg_maritime-compat/ready)
