(ns kongsberg_maritime.main-test
  "Contract + behavioral test for the kongsberg_maritime-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "kongsberg_maritime-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'kongsberg_maritime.main-test)]
